...
class Element():
	...
	def parse_element(self):
		...
		self.previous_sibling, self.first_child) = struct.unpack("<QQQQQ", _bytes)
		...
		self.node_info_ptrs = (node_info_ptrs[0], node_info_ptrs[2], node_info_ptrs[4])
		sizes = (node_info_ptrs[1], node_info_ptrs[3], node_info_ptrs[5]) 
	...
	def __str__(self):
		if self.content is None:
			return "[{}] element at {}".format(self.node_info_strs, hex(self.addr))
		return "[{}] element at {} --> {}".format(self.node_info_strs, hex(self.addr), self.content)
